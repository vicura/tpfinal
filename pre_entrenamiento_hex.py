...
"""Para que la red reconozca cada mesofase que adopta el sistema, le doy en el entrenamiento varias muestras de cada una: hexelar desordenada, hexelar y hexelar ordenada.


Estas muestras consisten en outputs .hexmpstj de corridas en condiciones constantes de presión y temperatura (NPT). Para cada mesofase tengo 4 outputs correspondientes a 4 temperaturas distintas pero cercanas.
...
def main():
    ...
    iso = [d for d in list_dir if d.startswith('hex_dump_desord')]
    ...
    hex = [d for d in list_dir if d.startswith('ord_dump_hex')]          
